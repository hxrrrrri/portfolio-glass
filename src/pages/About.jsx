      ...
      <div className="page">
        ...
        <div className="ab-hero" style={{display:'grid',gridTemplateColumns:'1fr 1.85fr',gap:'18px',marginBottom:'18px'}}>
          <div className="g1 g-hi2 g-shine" style={{padding:'56px',borderRadius:'28px',display:'flex',flexDirection:'column',justifyContent:'space-between',minHeight:'340px',position:'relative',overflow:'hidden'}}>
            <div style={{position:'absolute',bottom:0,left:0,width:'280px',height:'280px',background:'radial-gradient(circle at 20% 80%,rgba(var(--a-r),var(--a-g),var(--a-b),0.06),transparent 65%)',pointerEvents:'none'}}/>
            <div>
              <div className="text-label" style={{marginBottom:'22px'}}>X / Labs</div>
              <h1 className="grad-text" style={{fontSize:'clamp(4rem,8vw,8rem)',fontWeight:900,lineHeight:0.85,letterSpacing:'-0.055em'}}>Lab<span style={{WebkitTextFillColor:'var(--a1)'}}>.</span></h1>
            </div>
          ...
          <div style={{display:'flex',flexDirection:'column',gap:'16px'}}>
            <div className="about-accent-bar" style={{padding:'22px 32px',borderRadius:'20px'}}>
            ...
            <div ref={conRef} onMouseMove={onMove} onMouseLeave={onLeave} style={{flex:1,perspective:'1100px'}}>
              <div ref={cardRef} className="g2 g-hi2" style={{padding:'44px',borderRadius:'24px',height:'100%',transition:'transform 0.18s ease-out, box-shadow 0.3s',transformStyle:'preserve-3d',position:'relative',overflow:'hidden'}}>
                <div style={{position:'absolute',top:0,right:0,width:'200px',height:'200px',background:'radial-gradient(circle at 80% 20%,rgba(var(--a-r),var(--a-g),var(--a-b),0.07),transparent 65%)',pointerEvents:'none'}}/>
                <div style={{position:'relative',zIndex:1}}>
                  ...
                  <div style={{display:'flex',gap:'8px',flexWrap:'wrap'}}>
                    ...
                    <span style={{padding:'5px 14px',borderRadius:'999px',fontSize:'9px',fontWeight:800,letterSpacing:'0.10em',textTransform:'uppercase',background:'rgba(var(--a-r),var(--a-g),var(--a-b),0.16)',border:'1px solid rgba(var(--a-r),var(--a-g),var(--a-b),0.34)',color:'var(--a1)'}}>Open to Work</span>
                  </div>
                </div>
              </div>
            </div>
        ...
        <div className="ab-story g1 g-hi2" style={{padding:'56px',borderRadius:'28px',display:'grid',gridTemplateColumns:'340px 1fr',gap:'60px',marginBottom:'18px',position:'relative',overflow:'hidden'}}>
          <div style={{position:'absolute',top:0,left:0,width:'300px',height:'300px',background:'radial-gradient(circle at 10% 10%,rgba(var(--a-r),var(--a-g),var(--a-b),0.055),transparent 60%)',pointerEvents:'none'}}/>
          <div style={{position:'relative',zIndex:1}}>
            <div className="text-label" style={{marginBottom:'20px'}}>+002 / Story</div>
            <h2 style={{fontSize:'clamp(2.2rem,3.5vw,3.8rem)',fontWeight:900,lineHeight:0.93,letterSpacing:'-0.045em',color:'rgba(255,255,255,0.97)'}}>Creative<br/>Problem<br/>Solver.</h2>
          </div>
          <div style={{display:'flex',flexDirection:'column',gap:'20px',justifyContent:'center',position:'relative',zIndex:1}}>
            {['Final year CSE (AI) student at Mar Baselios College of Engineering and Technology, Trivandrum. I build full-stack AI platforms — from satellite imagery analysis to autonomous robotics.',
              ...
              .map((p,i)=><p key={i} style={{fontSize:'14px',color:'rgba(255,255,255,0.48)',lineHeight:1.82,borderLeft:i===0?'2px solid rgba(var(--a-r),var(--a-g),var(--a-b),0.40)':'none',paddingLeft:i===0?'18px':'0'}}>{p}</p>)}
          </div>
        </div>
            ...
            {[{label:'Frontend & Web',items:skills.frontend},{label:'Backend & Infra',items:skills.backend},{label:'AI / ML',items:skills.ai}]
              .map((b,bi)=>(
                <div key={b.label} className="sk-block g1 g-shine g-hi card-lift" style={{padding:'38px',borderRadius:'24px',position:'relative',overflow:'hidden'}}>
                  <div style={{position:'absolute',top:0,right:0,width:'150px',height:'150px',background:`radial-gradient(circle at 80% 20%, ${bi===0?'rgba(var(--a-r),var(--a-g),var(--a-b),0.06)':bi===1?'rgba(80,40,255,0.06)':'rgba(255,180,40,0.05)'}, transparent 65%)`,pointerEvents:'none'}}/>
        ...
        <div className="g1 g-hi2" style={{padding:'48px 56px',borderRadius:'28px',position:'relative',overflow:'hidden'}}>
          <div style={{position:'absolute',top:0,right:0,width:'400px',height:'250px',background:'radial-gradient(ellipse at 80% 30%,rgba(var(--a-r),var(--a-g),var(--a-b),0.05),transparent 65%)',pointerEvents:'none'}}/>
          ...
          {[{label:'GitHub',href:socials.github},{label:'LinkedIn',href:socials.linkedin},{label:'Email',href:`mailto:${socials.email}`}].map(({label,href},i,a)=>(
            <a key={label} href={href} target={label!=='Email'?'_blank':undefined} rel="noopener noreferrer"
              ...
              onMouseEnter={e=>{e.currentTarget.style.color='var(--a1)';e.currentTarget.style.letterSpacing='-0.01em'}}
